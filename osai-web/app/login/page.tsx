...
export default function LoginPage() {
  ...
  // Demo workspace = shared sample data, no real account. Real sign-in is Google
  // only — there is no passwordless email login.
  function enterDemo() {
    localStorage.setItem("osai_token", "demo-token");
    localStorage.setItem("osai_org_id", "demo-org");
    localStorage.setItem("osai_org_name", "OSAI Demo");
    localStorage.setItem("osai_user_email", "demo@osai.local");
    localStorage.setItem("osai_user_name", "Demo User");
    router.replace("/demo");
  }
      ...
      <div className="login-card">
        ...
        {googleEnabled ? (
          ...
          <div className="login-error">
            <span>⚠</span> Sign-in isn&apos;t configured yet. Add Google OAuth credentials to enable login.
          </div>
        )}
