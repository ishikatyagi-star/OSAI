...
Exposes a small set of Composio actions as JSON-schema tools the reasoning
layer can call after user confirmation.
"""
...
def available_action_tools() -> dict[str, dict[str, Any]]:
    """External action tools enabled by the Composio integration."""
    return dict(_ACTION_TOOLS) if settings.composio_api_key else {}
...
def build_payload(tool_name: str, params: dict[str, Any]) -> dict[str, Any]:
    """Normalize planner params into a Composio tool payload."""
    params = params or {}
